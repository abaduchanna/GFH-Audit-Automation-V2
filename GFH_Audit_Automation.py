...
class GFHApp(tk.Tk):
    def __init__(self):
        ...
        self.scheduled_time_var = tk.StringVar(value="09:00")
        self.scheduler_status_var = tk.StringVar(value="DISABLED")
        
        # B2B Soft Login
        self.b2b_access_code_var = tk.StringVar(value="9909129")
        self.b2b_account_id_var = tk.StringVar(value="")
        self.b2b_username_var = tk.StringVar(value="")
        self.b2b_password_var = tk.StringVar(value="")
    ...
    def _build_ui(self) -> None:
        ...
        # ── SCHEDULER PANEL ──
        scheduler_box = ttk.LabelFrame(root, text="⏰ Scheduler Configuration", padding=10)
        scheduler_box.pack(fill="x", pady=(12, 8))
        
        ttk.Label(scheduler_box, text="Trigger Time (HH:MM):").grid(row=0, column=0, sticky="w", pady=3)
        ttk.Entry(scheduler_box, textvariable=self.scheduled_time_var, width=15).grid(row=0, column=1, sticky="w", pady=3)
        
        ttk.Label(scheduler_box, text="Status:").grid(row=1, column=0, sticky="w", pady=3)
        ttk.Label(scheduler_box, textvariable=self.scheduler_status_var, font=("Segoe UI", 9, "bold"), foreground="red").grid(row=1, column=1, sticky="w", pady=3)
        
        scheduler_btn_frame = ttk.Frame(scheduler_box)
        scheduler_btn_frame.grid(row=2, column=0, columnspan=2, sticky="ew", pady=8)
        ttk.Button(scheduler_btn_frame, text="✓ Enable Scheduler", command=self.enable_scheduler, width=18).pack(side="left", padx=2)
        ttk.Button(scheduler_btn_frame, text="✗ Disable", command=self.disable_scheduler, width=12).pack(side="left", padx=2)

        # ── B2B SOFT LOGIN PANEL ──
        b2b_box = ttk.LabelFrame(root, text="🔐 B2B Portal Login", padding=10)
        b2b_box.pack(fill="x", pady=(0, 8))
        
        ttk.Label(b2b_box, text="Access Code:").grid(row=0, column=0, sticky="w", pady=3)
        ttk.Entry(b2b_box, textvariable=self.b2b_access_code_var, width=25).grid(row=0, column=1, sticky="ew", pady=3)
        
        ttk.Label(b2b_box, text="Account ID:").grid(row=1, column=0, sticky="w", pady=3)
        ttk.Entry(b2b_box, textvariable=self.b2b_account_id_var, width=25).grid(row=1, column=1, sticky="ew", pady=3)
        
        ttk.Label(b2b_box, text="Username:").grid(row=2, column=0, sticky="w", pady=3)
        ttk.Entry(b2b_box, textvariable=self.b2b_username_var, width=25).grid(row=2, column=1, sticky="ew", pady=3)
        
        ttk.Label(b2b_box, text="Password:").grid(row=3, column=0, sticky="w", pady=3)
        ttk.Entry(b2b_box, textvariable=self.b2b_password_var, show="•", width=25).grid(row=3, column=1, sticky="ew", pady=3)
        
        b2b_btn_frame = ttk.Frame(b2b_box)
        b2b_btn_frame.grid(row=4, column=0, columnspan=2, sticky="ew", pady=8)
        ttk.Button(b2b_btn_frame, text="Save", command=self.save_b2b_credentials, width=10).pack(side="left", padx=2)
        ttk.Button(b2b_btn_frame, text="Test", command=self.test_b2b_login, width=10).pack(side="left", padx=2)
        ttk.Button(b2b_btn_frame, text="Auto-Fetch", command=self.auto_fetch_b2b, width=12).pack(side="left", padx=2)

        # ── GFH TIMESHEET LOGIN PANEL ──
        ts_box = ttk.LabelFrame(root, text="👥 GFH Timesheet Login", padding=10)
        ts_box.pack(fill="x", pady=(0, 12))
        
        ttk.Label(ts_box, text="Email:").grid(row=0, column=0, sticky="w", pady=3)
        ttk.Entry(ts_box, textvariable=self.ts_email_var, width=25).grid(row=0, column=1, sticky="ew", pady=3)
        
        ttk.Label(ts_box, text="Password:").grid(row=1, column=0, sticky="w", pady=3)
        ttk.Entry(ts_box, textvariable=self.ts_password_var, show="•", width=25).grid(row=1, column=1, sticky="ew", pady=3)
        
        ts_btn_frame = ttk.Frame(ts_box)
        ts_btn_frame.grid(row=2, column=0, columnspan=2, sticky="ew", pady=8)
        ttk.Button(ts_btn_frame, text="Save", command=self.save_ts_credentials, width=10).pack(side="left", padx=2)
        ttk.Button(ts_btn_frame, text="Test", command=self.test_ts_login, width=10).pack(side="left", padx=2)
        ttk.Button(ts_btn_frame, text="Auto-Fetch", command=self.auto_fetch_timesheet, width=12).pack(side="left", padx=2)

        # ── AUTO-IMPORT STATUS ──
        file_box = ttk.LabelFrame(root, text="📊 Auto-Import Status", padding=10)
        file_box.pack(fill="x", pady=(0, 8))
        ttk.Label(file_box, text="B2B Inventory:").grid(row=0, column=0, sticky="w", pady=3)
        ttk.Entry(file_box, textvariable=self.inventory_path).grid(row=0, column=1, sticky="ew", pady=3)
        ttk.Label(file_box, text="Timesheet:").grid(row=1, column=0, sticky="w", pady=3)
        ttk.Entry(file_box, textvariable=self.time_sheet_path).grid(row=1, column=1, sticky="ew", pady=3)
    ...
    def enable_scheduler(self) -> None:
        ...
        try:
            ...
            if not hasattr(self, 'scheduler') or self.scheduler is None:
                time_str = self.scheduled_time_var.get()
                self.scheduler = AuditScheduler(self, check_interval=30)
                self.scheduler.set_time(time_str)
                self.scheduler.set_trigger_callback(self.on_scheduled_trigger)
                self.scheduler.start()
                
                self.scheduler_enabled = True
                self.scheduler_status_var.set(f"ENABLED - {time_str}")
                self.set_status(f"✓ Scheduler enabled - triggers at {time_str}")
            else:
            ...
            self.set_status(f"❌ Scheduler error: {str(e)}")
    ...
    def disable_scheduler(self) -> None:
        ...
        try:
            ...
            self.set_status("✓ Scheduler disabled")
        except Exception as e:
            logger.error(f"Scheduler disable error: {e}")
    
    def on_scheduled_trigger(self) -> None:
        """Called when scheduler time is reached"""
        try:
            from audit_scheduler import ReminderScheduler
            from datetime import datetime
            import time
            
            self.set_status("🚀 Scheduled audit triggered!")
            
            # Fetch both files
            self.auto_fetch_b2b()
            time.sleep(2)
            self.auto_fetch_timesheet()
            time.sleep(2)
            
            # Load and process variances
            if hasattr(self, 'load_variances'):
                self.load_variances()
            
            # Start reminder scheduler
            self.reminder_scheduler = ReminderScheduler(self)
            self.reminder_scheduler.schedule_reminders(datetime.now(), interval_minutes=45)
            self.reminder_scheduler.set_reminder_callback(self.on_reminder)
            
            # Start OCR listener
            self.start_ocr_listener()
            
            self.set_status("✓ Audit workflow started - monitoring for updates")
            
        except Exception as e:
            logger.exception(f"Scheduled trigger error: {e}")
            self.set_status(f"❌ Trigger error: {str(e)}")
    
    def on_reminder(self, reminder_num: int) -> None:
        """Called for each escalation reminder"""
        try:
            msg = f"⏰ *Reminder {reminder_num}/3*\n\nPlease submit your inventory updates"
            
            # Send via WhatsApp if available
            if hasattr(self, 'send_message_to_group'):
                self.send_message_to_group(msg)
            
            self.set_status(f"📨 Sent reminder {reminder_num}/3")
            
        except Exception as e:
            logger.error(f"Reminder send error: {e}")
            self.set_status(f"⚠️ Reminder {reminder_num} send failed")
    
    def start_ocr_listener(self) -> None:
        """Start OCR listener for WhatsApp images"""
        try:
            from ocr_engine import OCREngine, VarianceReconciler
            import threading
            ...
            self.variance_reconciler = VarianceReconciler()
            
            # Start background thread
            self.ocr_thread = threading.Thread(
                target=self._ocr_monitor_loop,
                daemon=True
            )
            self.ocr_thread.start()
            self.set_status("👁️ OCR listener started")
            
        except Exception as e:
            logger.error(f"OCR listener start error: {e}")
            self.set_status(f"❌ OCR error: {str(e)}")
    
    def _ocr_monitor_loop(self) -> None:
        """Background OCR monitoring loop"""
        try:
            from pathlib import Path
            import time
            
            # WhatsApp images folder
            whatsapp_dir = Path.home() / "Pictures" / "WhatsApp" / "Media" / "WhatsApp Images"
            if not whatsapp_dir.exists():
                whatsapp_dir = Path.home() / "Downloads" / "WhatsApp"
            ...
            while True:
                try:
                    ...
                    for image_path in whatsapp_dir.glob("*.png"):
                        if image_path.name not in processed:
                            # Extract IMEIs
                            imeis = self.ocr_engine.extract_imeis_from_image(str(image_path))
                            
                            if imeis:
                                # Match against variances
                                matches = self.variance_reconciler.reconcile_imeis(imeis)
                                
                                # Auto-deduct cleared items
                                for imei in matches['cleared']:
                                    self.deduct_variance_item(imei)
                                
                                # Status update
                                cleared = len(matches['cleared'])
                                not_found = len(matches['not_found'])
                                self.set_status(
                                    f"✓ OCR: {cleared} cleared, {not_found} not found"
                                )
                    ...
                    if hasattr(self, 'reminder_scheduler'):
                        self.reminder_scheduler.check_reminders()
                        
                except Exception as e:
                    logger.error(f"OCR loop iteration error: {e}")
                    time.sleep(10)
        except Exception as e:
            logger.error(f"OCR monitor loop error: {e}")
    
    def deduct_variance_item(self, imei: str) -> None:
        """Remove IMEI from variance list"""
        try:
            # Find and remove from current variance table
            if hasattr(self, 'variance_table'):
                for row in self.variance_table.get_children():
                    values = self.variance_table.item(row)['values']
                    if imei in str(values):
                        self.variance_table.delete(row)
                        logger.info(f"✓ Deducted IMEI: {imei}")
                        break
        except Exception as e:
    ...
    def save_b2b_credentials(self) -> None:
        """Save B2B credentials"""
        access_code = self.b2b_access_code_var.get().strip()
        account_id = self.b2b_account_id_var.get().strip()
        username = self.b2b_username_var.get().strip()
        password = self.b2b_password_var.get().strip()
        
        if not all([access_code, account_id, username, password]):
            messagebox.showerror("Validation", "All B2B fields are required")
            return
        
        self.set_status("✓ B2B credentials saved")
    
    def test_b2b_login(self) -> None:
        """Test B2B login"""
        self.set_status("🔐 Testing B2B login...")
        messagebox.showinfo("Info", "B2B login test - TODO: Implement Selenium scraper")
    
    def auto_fetch_b2b(self) -> None:
        """Auto-fetch B2B inventory count"""
        try:
            from b2b_scraper import scrape_b2b_inventory
            from pathlib import Path
            
            self.set_status("📊 Auto-fetching B2B inventory...")
            success, msg, filepath = scrape_b2b_inventory(
                self.b2b_access_code_var.get(),
                self.b2b_account_id_var.get(),
                self.b2b_username_var.get(),
                self.b2b_password_var.get()
            )
            if success and filepath:
                self.inventory_path.set(filepath)
                self.set_status(f"✓ B2B: {Path(filepath).name}")
            else:
                self.set_status(f"❌ B2B: {msg}")
        except Exception as e:
            logger.error(f"B2B fetch error: {e}")
            self.set_status(f"❌ B2B error: {str(e)}")
    
    # ======================== TIMESHEET METHODS ========================
    
    def save_ts_credentials(self) -> None:
        """Save timesheet credentials"""
        email = self.ts_email_var.get().strip()
        password = self.ts_password_var.get().strip()
        
        if not email or not password:
            messagebox.showerror("Validation", "Email and password are required")
            return
        
        self.set_status("✓ Timesheet credentials saved")
    
    def test_ts_login(self) -> None:
        """Test timesheet login"""
        self.set_status("🔐 Testing timesheet login...")
        messagebox.showinfo("Info", "Timesheet login test - TODO: Implement BeautifulSoup scraper")
    
    def auto_fetch_timesheet(self) -> None:
        """Auto-fetch timesheet data"""
        try:
            from gfh_timesheet_scraper import scrape_gfh_timesheet
            from pathlib import Path
            
            self.set_status("👥 Auto-fetching timesheet...")
            success, msg, filepath = scrape_gfh_timesheet(
                self.ts_email_var.get(),
                self.ts_password_var.get()
            )
            if success and filepath:
                self.time_sheet_path.set(filepath)
                self.set_status(f"✓ Timesheet: {Path(filepath).name}")
            else:
                self.set_status(f"❌ Timesheet: {msg}")
        except Exception as e:
            logger.error(f"Timesheet fetch error: {e}")
            self.set_status(f"❌ Timesheet error: {str(e)}")
